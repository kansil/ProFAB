...
class classifiers(object):
    """
    In class,6 different machine learning methods for regression are introduced. Their hyperparameters are tuned by
    ...
    def logistic_regression(self,X_train,y_train):    
        ...
        c = np.linspace(0.001,100, num=1000)
    ...
    def SVM(self,X_train,y_train):
        ...
        self.parameters = dict(kernel = kernel,nu=nu,gamma = gamma,max_iter = max_iter)
        model = NuSVC()
        self.get_best_model(X_train,y_train,model)
    ...
    def naive_bayes(self,X_train,y_train):
        ...
        best_model = model
        pickle.dump(best_model, open('Model_Data_Classification', 'wb'))
    ...
    def decision_tree(self,X_train,y_train):
        ...
        self.parameters = dict(max_feature = max_feature,max_depth = max_depth,
                               criterion = criterion)
    ...
    def gradient_boosting(self,X_train,y_train):
        ...
        self.parameters = {loss:loss,learning_rate:learning_rate,n_estimators:n_estimators,criterion:criterion,max_depth:max_depth}
        model = GBC()
        self.get_best_model(X_train,y_train,model)

def classification_methods(path, ml_type,X_train,y_train,cv):
