...
def clssfy_data_import(pos_file,neg_file, label, pos_indices = None,neg_indices = None):
    ...
    if pos_indices != None and neg_indices != None:
        ...
        for k,(rowx) in enumerate(pf):
            pos_idx = set(np.loadtxt(pos_indices_file))
            neg_idx = set(np.loadtxt(neg_indices_file))
            if k in pos_idx:
                rowx = re.split(' ',rowx.strip('\n'))
                rowx = list(np.array(rowx,dtype = 'float64'))
                if label == 'positive':
...
def form_table(score_path, names, scores, sizes, learning_type, preds):
    ...
    if learning_type == 'Regression':
        ...
        if len(names) == 2:            
            ...
            train = [names[0]] + [sizes[0]] + list(scores[0].keys())[:-1] + th_train

            th_test = list(scores[1]['threshold based Metrics'].values())
            test = [names[1]] + [sizes[1]] + list(scores[1].values)[:-1] + th_test          
            ...
            validation = [names[2]] + [sizes[2]] + list(scores[2].values)[:-1] + th_validation          
        ...
        if len(names) == 2:            
            ...
            train = [names[0]] + [sizes[0]] + list(scores[0].values())
            ...
            test = [names[1]] + [sizes[1]] + list(scores[1].values())

            validation = [names[2]] + [sizes[2]] + list(scores[2].values())
