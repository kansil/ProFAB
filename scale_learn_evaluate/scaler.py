...
def scaling(X_train,scale_type):
    
    s = ScalerNormalizer()

    scaler_methods = {'Normalizer':s.normalizer,'Standard_Scaler':s.standard_scaler,
                   'MaxAbs_Scaler':s.maxabs_scaler,'MinMax_Scaler': s.minmax_scaler,
                   'Robust Scaler': s.robust_scaler}
    
    X_train,scaler = scaler_methods[scale_type](X_train)
