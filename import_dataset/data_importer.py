...
class cls_data_loader():
    ...
    def __init__(self,ratio,protein_feature,main_name,set_type,label,pre_determined):
        self.ratio = ratio
        self.protein_feature = protein_feature
        self.set_type = set_type
        self.label = label
    ...
    def get_data(self,data_name):
        pPath = os.path.split(os.path.realpath(__file__))[0]
        file_path = pPath + '/' + self.main_name + '/' + data_name
        ...
        if not self.pre_determined:
            ...
            pX,py,nX,ny,X,y = clssfy_data_import(pos_file = pos_file,pos_file = neg_file, label = self.label)
            ...
            train_pos_idx = file_path + '/' + self.set_type + '_positive_train.txt'
            train_neg_idx = file_path + '/' + self.set_type + '_negative_train.txt'

            test_pos_idx = file_path + '/' + self.set_type + '_positive_test.txt'
            test_neg_idx = file_path + '/' + self.set_type + '_negative_test.txt'



            data_files = [pos_file, neg_file, train_pos_idx, train_neg_idx, test_pos_idx, trest_neg_idx]
